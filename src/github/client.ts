...
export async function fetchRepoPRs(
  ...
  return results.filter((p): p is PullRequest => p !== null)
}
...
export async function buildRepoData(
  ...
  const prs = await fetchRepoPRs(octokit, owner, repoName, prNumbers)
  const techStack = extractTechStack(prs)
  return { name: repoName, org: owner, company, period, techStack, prs }
}
